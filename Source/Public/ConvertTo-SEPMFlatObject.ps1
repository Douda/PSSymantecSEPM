...
Function ConvertTo-SEPMFlatObject {
    <#
    .SYNOPSIS
    ...
    .PARAMETER Depth
    The maximal depth of flattening a recursive property. Any negative value will result in an unlimited depth and could cause an infinite loop.
    ...
    $Object3 | ConvertTo-SEPMFlatObject
    ...
    End {
        ...
        If ($PSBoundParameters.ContainsKey("OutputObject")) {
            ...
            $Iterate = [ordered] @{}
            if ($null -eq $Object) {
                #Write-Verbose -Message "ConvertTo-SEPMFlatObject - Object is null"
            ...
            If ($Iterate.Keys.Count) {
                foreach ($Key in $Iterate.Keys) {
                    if ($Key -notin $ExcludeProperty) {
                        ConvertTo-SEPMFlatObject -Objects @(, $Iterate["$Key"]) -Separator $Separator -Base $Base -Depth $Depth -Path ($Path + $Key) -OutputObject $OutputObject -ExcludeProperty $ExcludeProperty
                    }
                }
            } else {
            ...
            foreach ($ItemObject in $InputObjects) {
                ...
                ConvertTo-SEPMFlatObject -Objects @(, $ItemObject) -Separator $Separator -Base $Base -Depth $Depth -Path $Path -OutputObject $OutputObject -ExcludeProperty $ExcludeProperty
