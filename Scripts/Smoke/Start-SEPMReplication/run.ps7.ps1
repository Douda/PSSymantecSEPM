<#
.SYNOPSIS
    PS7 entry point for Start-SEPMReplication smoke tests.

.DESCRIPTION
    Bootstraps the module and SEPM connection for PS 7+, then dot-sources
    Common.ps1 (auth + helpers) and Tests.ps1 (test cases).

    Usage: pwsh -NoProfile -File Scripts/Smoke/Start-SEPMReplication/run.ps7.ps1
#>
...
. "$RepoRoot/Scripts/Smoke/Start-SEPMReplication/Tests.ps1"
