<#
.SYNOPSIS
    PS7 entry point for Get-SEPMPoliciesSummary smoke tests.

.DESCRIPTION
    Bootstraps the module and SEPM connection for PS 7+, then dot-sources
    Common.ps1 (auth + helpers) and Tests.ps1 (test cases).

    Usage: pwsh -NoProfile -File Scripts/Smoke/Get-SEPMPoliciesSummary/run.ps7.ps1
#>
...
. "$RepoRoot/Scripts/Smoke/Get-SEPMPoliciesSummary/Tests.ps1"
