<#
.SYNOPSIS
    PS7 entry point for Move-SEPClientGroup smoke tests.

.DESCRIPTION
    Bootstraps the module and SEPM connection for PS 7+, then dot-sources
    Common.ps1 (auth + helpers) and Tests.ps1 (test cases).

    Usage: pwsh -NoProfile -File Scripts/Smoke/Move-SEPClientGroup/run.ps7.ps1
#>
...
. "$RepoRoot/Scripts/Smoke/Move-SEPClientGroup/Tests.ps1"
