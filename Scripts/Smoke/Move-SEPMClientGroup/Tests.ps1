<#
.SYNOPSIS
    Shared smoke tests for Move-SEPMClientGroup.
...
if (-not $computers -or $computers.Count -eq 0) {
    ...
    if (-not $srcComputer) {
        ...
        $results.A1 = T "A1" "Move computer" `
            { Move-SEPMClientGroup -ComputerName $srcComputer -GroupName $targetGroup } `
            { param($r) $r -and $r.computerName }

        # ── A2: Move back ──
        Start-Sleep -Seconds 1
        $results.A2 = T "A2" "Move back" `
            { Move-SEPMClientGroup -ComputerName $srcComputer -GroupName $srcGroup } `
            { param($r) $r -and $r.computerName }
        ...
        $results.A3 = T "A3" "Output type" `
            {
                Start-Sleep -Seconds 1
                $r = Move-SEPMClientGroup -ComputerName $srcComputer -GroupName $targetGroup
                Start-Sleep -Seconds 1
                Move-SEPMClientGroup -ComputerName $srcComputer -GroupName $srcGroup | Out-Null
        ...
        $results.A5 = T "A5" "Error on invalid" `
            {
                $errs = $null
                Move-SEPMClientGroup -ComputerName 'NonExistentComputerXYZ' -GroupName $targetGroup -ErrorVariable errs -ErrorAction SilentlyContinue
                $errs.Count -gt 0
            } `
            { param($r) $r -eq $true }
    }
...
# ── Summary ──
Write-Summary -Results $results -Label "Move-SEPMClientGroup"
