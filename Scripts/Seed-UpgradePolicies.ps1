<#
.SYNOPSIS
    Seeds SEPM with Upgrade Policies.

.DESCRIPTION
    Reads Source/Seed/UpgradePolicies.psd1 and creates Upgrade policies
    on the SEPM server via POST to /api/v1/policies/upgrade followed by
...
.EXAMPLE
    Invoke-SeedUpgradePolicies -State @{ Force = $false; Session = $session }

    Creates all seed Upgrade policies (idempotent).

.EXAMPLE
    Invoke-SeedUpgradePolicies -State @{ Force = $true; Session = $session }

    Deletes existing seed Upgrade policies, then recreates.
#>

#Requires -Version 5.1

function Invoke-SeedUpgradePolicies {
    [CmdletBinding()]
    param(
    ...
    # Load the UpgradePolicies data file
    $scriptDir = Split-Path -Path $PSScriptRoot -Parent
    if (Test-Path (Join-Path -Path $scriptDir -ChildPath 'Source/Seed/UpgradePolicies.psd1')) {
        $seedDir = Join-Path -Path $scriptDir -ChildPath 'Source/Seed'
    } else {
        $seedDir = Join-Path -Path $PSScriptRoot -ChildPath 'Source/Seed'
    }
    $data = Import-PowerShellDataFile -Path (Join-Path -Path $seedDir -ChildPath 'UpgradePolicies.psd1') -ErrorAction Stop
    ...
    if ($State.Force) {
        ...
        $currentPolicies = _InvokeApi -Method GET -Uri "$($session.BaseURLv1)/policies/summary/upgrade" -Session $session
        ...
        foreach ($policyName in $seedNames) {
            ...
            if ($existing -and $existing.id) {
                ...
                _InvokeApi -Method PATCH -Uri "$($session.BaseURLv1)/policies/upgrade/$($existing.id)" `
                    -Session $session -Body $disableBody

                _InvokeApi -Method DELETE -Uri "$($session.BaseURLv1)/policies/upgrade/$($existing.id)" `
                    -Session $session
            }
        }
    }
    ...
    $existingPolicies = _InvokeApi -Method GET -Uri "$($session.BaseURLv1)/policies/summary/upgrade" -Session $session
    ...
    foreach ($entry in $data.Policies) {
        ...
        $createUri = "$($session.BaseURLv1)/policies/upgrade"
        ...
        $summaryResp = _InvokeApi -Method GET -Uri "$($session.BaseURLv1)/policies/summary/upgrade" -Session $session
    ...
    # Merge UpgradePolicyMap into State
    $State['UpgradePolicyMap'] = $policyMap
    return $State
}
