...
function Invoke-SeedUpgradePolicies {
    ...
    if ($State.Force) {
        ...
        $currentPolicies = _InvokeApi -Method GET -Uri "$baseUrl/policies/summary/upgrade" -Session $session
        ...
        foreach ($policyName in $seedNames) {
            ...
            if ($existing -and $existing.id) {
                ...
                _InvokeApi -Method PATCH -Uri "$baseUrl/policies/upgrade/$($existing.id)" `
                    -Session $session -Body $disableBody

                _InvokeApi -Method DELETE -Uri "$baseUrl/policies/upgrade/$($existing.id)" `
                    -Session $session
            }
        }
    }
    ...
    $existingPolicies = _InvokeApi -Method GET -Uri "$baseUrl/policies/summary/upgrade" -Session $session
    ...
    foreach ($entry in $data.Policies) {
        ...
        $createUri = "$baseUrl/policies/upgrade"
        ...
        $summaryResp = _InvokeApi -Method GET -Uri "$baseUrl/policies/summary/upgrade" -Session $session
