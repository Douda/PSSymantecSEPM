...
.EXAMPLE
    Invoke-SeedTDADPolicies -State @{ Force = $false; Session = $session }

    Creates all seed TDAD policies (idempotent).

.EXAMPLE
    Invoke-SeedTDADPolicies -State @{ Force = $true; Session = $session }

    Deletes existing seed TDAD policies, then recreates.
#>

#Requires -Version 5.1

function Invoke-SeedTDADPolicies {
    [CmdletBinding()]
    param(
    ...
    # Load the TDADPolicies data file
    $scriptDir = Split-Path -Path $PSScriptRoot -Parent
    if (Test-Path (Join-Path -Path $scriptDir -ChildPath 'Source/Seed/TDADPolicies.psd1')) {
